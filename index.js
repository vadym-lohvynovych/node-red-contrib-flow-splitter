...
function writeSplitterConfig(cfg, projectPath) {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(projectPath, splitCfgFilename),
...
async function onFlowReload(flowEventData) {
  ...
  if (fs.existsSync(path.join(userDir || ".", ".config.projects.json"))) {
    ...
    projectPath = nrProjectsCfg.activeProject
  ...
  if (fs.existsSync(path.join(projectPath, splitCfgFilename))) {
    currentProjectSplitterCfg = JSON.parse(
      fs.readFileSync(path.join(projectPath, splitCfgFilename)),
    );
    currentProjectSplitterCfg.monolithFilename =
      currentProjectSplitterCfg.monolithFilename ||
  ...
  if (flowEventData.config.flows.length == 0) {
    ...
    currentProjectSplitterCfg = manager.constructMonolithFileFromFlowSet(
      flowSet,
      currentProjectSplitterCfg || DEFAULT_CFG,
      projectPath,
      false,
    ...
    flowSet = manager.constructFlowSetFromMonolithObject(
      flowEventData.config.flows,
    );

    currentProjectSplitterCfg = manager.constructTreeFilesFromFlowSet(
      flowSet,
      currentProjectSplitterCfg || DEFAULT_CFG,
      projectPath,
    );
